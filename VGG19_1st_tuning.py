...
from keras.layers import Dropout, Flatten, Dense,BatchNormalization, GaussianNoise, Input,Activation
...
def generate_predictions_file(probabilities, thresholds,label_list):
    ...
    proportion_ones = [7, 93, 31, 71, 19, 10, 20, 12, 1, 6, 18, 1, 3, 1, 1, 1, 1]
...
def run_model(df,proportions,arch,tag,tag_weights,augmented,image_folder,norm,filetype,size,channels,label_list):
    ...
    for train_index, test_index in kf.split(data_set):
        ...
        weight_file = 'C:/planet/saved_models/weights.best_' + tag + '_' + 'VGG19' + '_' + str(fold) + '.hdf5'

        # Create the model
        from keras.applications.vgg19 import VGG19

        input_tensor = Input(shape=train_set.shape[1:])
    ...
    for fold in range(nsplit):
        ...
        data_probs[fold,:,:] =  final_model.predict(valid_set)
